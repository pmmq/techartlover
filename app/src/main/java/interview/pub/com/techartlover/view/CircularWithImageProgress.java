package interview.pub.com.techartlover.view;
...
public class CircularWithImageProgress extends View {
    ...
    private static final String INISTNCE_IMG_SIZE = "img_size";
    ...
    public CircularWithImageProgress(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        min_size = (int) ConvertUtils.dp2px(getResources(), 100);
        default_stroke_width = ConvertUtils.dp2px(getResources(), 15);
        default_inner_bottom_text_size = ConvertUtils.sp2px(getResources(), 18);
    ...
    public void setCenterImage(Bitmap image) {
        centerImage = image;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (centerImage == null) {
            ...
            Rect frameToDraw = new Rect(0, 0, centerImage.getWidth() - 1, centerImage.getHeight() - 1);

            int xStart = canvas.getWidth() * 30 / 100;
            int yStart = canvas.getHeight() * 30 / 100;
            int xEnd = xStart + (canvas.getWidth() - (canvas.getWidth() * 60 / 100));
            int yEnd = yStart + (canvas.getHeight() - (canvas.getHeight() * 60 / 100));

            Rect whereToDraw = new Rect(xStart, yStart, xEnd, yEnd);

            canvas.drawBitmap(centerImage, frameToDraw, whereToDraw, null);
        }
    }
